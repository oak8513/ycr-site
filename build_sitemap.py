"""
build_sitemap.py — Regenerates sitemap.xml from the actual files in the repo.
...
Excludes Jekyll source (_posts, _includes, _layouts), assets, mc.html, quote/.
Uses each file's mtime as <lastmod> in YYYY-MM-DD.

Run manually:   python build_sitemap.py
Auto-runs from: publish_post.py and the .git/hooks/pre-commit hook.

Idempotent. Writes Unix line endings + trailing newline to keep git diffs clean.
"""
...
SITE_URL = "https://yorkcomputerrepair.com"
SITEMAP_PATH = SITE_DIR / "sitemap.xml"

EXCLUDE_FILES = {"mc.html", "404.html"}
EXCLUDE_DIRS = {"_posts", "_includes", "_layouts", "_site", "assets",
                "__pycache__", ".git", "node_modules", "quote"}
...
def collect_urls() -> list[str]:
    ...
    # 2. Top-level always-on pages with custom priorities
    top_level = [
    ...
    # 3. /services/*.html
    services_dir = SITE_DIR / "services"
    if services_dir.is_dir():
        for f in sorted(services_dir.glob("*.html")):
            if f.name in EXCLUDE_FILES:
                continue
            entries.append(_entry(f"/services/{f.name}",
